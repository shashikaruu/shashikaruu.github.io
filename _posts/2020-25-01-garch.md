...
```r
# Rolling Model Forecast AR(5), GARCH(1,1)  - std dist.

f.period    = 173
h           = 5
f.error     = rep(NA, f.period-h)

spec.garch11   = ugarchspec(mean.model = list(armaOrder = c(5, 0), include.mean = TRUE),
                         variance.model = list(model = "sGARCH", garchOrder = c(1,1),external.regressors=dayoff.difference),
